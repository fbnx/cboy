...
MMU::MMU() 
    : m_ram{}, m_vram_bank{}, m_wram{}, m_bg_palette{}, m_sprite_palette{} {
}
...
void MMU::write(uint16_t addr, uint8_t value) {
    ...
    if (addr == 0xFF00) {
        ...
        if (buttons_selected && !directions_selected) {
            value |= gameboy::instance().get_controls() >> 4;
        } else if (directions_selected && !buttons_selected) {
            value |= gameboy::instance().get_controls() & 0xF;
        } else {
            value |= 0xF;
        }
    ...
    if (addr == 0xFF02) {
        // Serial output
        gameboy::instance().serial_print(m_ram[0xFF01 - 0x8000]);
        return;
    }
    ...
    if (addr == 0xFF55) {
        // CGB DMA
        uint16_t source = (read(0xFF51) << 8) | read(0xFF52);
        uint16_t target = (read(0xFF53) << 8) | read(0xFF54);
...
uint32_t MMU::get_bg_palette(uint8_t index) const {
    return *(uint32_t*)(m_bg_palette.data() + index * sizeof(uint32_t));
}

uint32_t MMU::get_sprite_palette(uint8_t index) const {
    return *(uint32_t*)(m_sprite_palette.data() + index * sizeof(uint32_t));
}

void MMU::set_interrupt(uint8_t value) {
    write(0xFF0F, read(0xFF0F) | (1 << value));
}
