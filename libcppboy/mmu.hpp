...
extern MMU mmu_instance;

// Convenience functions
inline uint8_t read(uint16_t addr) {
    return mmu_instance.read(addr);
}

inline void write(uint16_t addr, uint8_t value) {
    mmu_instance.write(addr, value);
}

inline void set_interrupt(uint8_t value) {
    mmu_instance.set_interrupt(value);
}

inline void set_vblank() {
    mmu_instance.set_vblank();
}
